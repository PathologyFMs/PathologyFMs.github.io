...
|---|---|---|---|---|
| **CTransPath** | 2022 | 32K+ WSIs | Hybrid CNN–Swin Transformer encoder pretrained with Semantically-Relevant Contrastive Learning (SRCL): beyond augmentation-based positives, it adds semantically similar 'pseudo-positive' patches retrieved from a memory bank, enriching histopathology representations | [Paper](https://www.sciencedirect.com/science/article/pii/S1361841522002043) • [Code](https://github.com/Xiyue-Wang/TransPath) • [Model](https://drive.google.com/file/d/1DoDx_70_TLj98gTf6YTXnu4tFhsFocDX/view) |
| **RetCCL** | 2023 | WSI retrieval (TCGA) | Clustering-guided contrastive learning (ResNet50) for whole-slide image retrieval; a general histopathology feature extractor from the CTransPath group | [Paper](https://doi.org/10.1016/j.media.2022.102645) • [Code](https://github.com/Xiyue-Wang/RetCCL) |
| **REMEDIS** | 2023 | 29K+ WSIs | BiT + SimCLR Pretraining | [Paper](https://arxiv.org/abs/2205.09723) |
| **Lunit** | 2023 | 36K+ WSI | Large-scale benchmark of self-supervised methods (MoCo v2, SwAV, Barlow Twins, DINO) pretrained on ~33M pathology patches (TCGA + Lunit's TULIP); shows domain-specific SSL beats ImageNet transfer and releases pathology-pretrained ResNet-50 / ViT-S encoders | [Paper](https://arxiv.org/abs/2212.04690) • [Code](https://github.com/lunit-io/benchmark-ssl-pathology) • [Model](https://huggingface.co/1aurent/vit_small_patch8_224.lunit_dino) |
...
| **BROW** | 2023 | 11K+ WSIs | Self-distillation (DINO-style) tile encoder that adds patch shuffling as an augmentation and pulls an extra global view from the WSI's own multi-scale pyramid, so the encoder sees slide-level context while still training on tiles | [Paper](https://arxiv.org/abs/2309.08259) • [Code](https://github.com/openmedlab/BROW) • [Model](https://drive.google.com/drive/folders/184TSs-rlzdttVWbRHIPCZgEYqIQNb9WB) |
...
| **RudolfV** | 2024 | 103,849 WSIs, 58 tissue types | Pathologist-curated data and expertise | [Paper](https://arxiv.org/abs/2401.04079) |
| **PLUTO** | 2024 | 195M tiles | Lightweight multi-scale representation | [Paper](https://arxiv.org/abs/2405.07905) |
| **UNI** | 2024 | 100M tiles, 100K WSIs | General-purpose tile encoder (ViT-Large) self-supervised with DINOv2 on Mass-100K (~100K H&E WSIs / 100M patches); transfers across 30+ clinical tasks via linear probing and few-shot, without task-specific training | [Paper](https://doi.org/10.1038/s41591-024-02857-3) • [Code](https://github.com/mahmoodlab/UNI) • [Model](https://huggingface.co/MahmoodLab/UNI) • [UNI2-h](https://huggingface.co/MahmoodLab/UNI2-h) • UNI2-g-preview |
| **Virchow** | 2024 | 1.5M WSIs, 100K patients | ViT-Huge (632M) tile encoder self-supervised with DINOv2 on 1.5M MSKCC H&E WSIs; a clinical-grade pan-cancer representation that notably improves rare-cancer detection | [Paper](https://doi.org/10.1038/s41591-024-03141-0) • [Model](https://huggingface.co/paige-ai/Virchow) |
| **Virchow2** | 2024 | 3.1M WSIs | Scales Virchow with pathology-specific SSL — mixed-magnification training (5×/10×/20×/40×) and an adapted DINOv2 on 3.1M diverse WSIs (multi-stain, multi-institution); released as ViT-H and a 1.9B-param Virchow2G variant | [Paper](https://arxiv.org/abs/2408.00738) • [Model](https://huggingface.co/paige-ai/Virchow2) • Virchow2G |
...
| **PathDino** | 2024 | Lightweight ViT (~9M params) | Lightweight rotation-agnostic histopathology ViT (~9M params, 5 blocks); HistoRotate 360-degree SSL + Fast Patch Selection for efficient WSI analysis | [Paper](https://arxiv.org/abs/2311.08359) • [Code](https://github.com/KimiaLabMayo/PathDino) |
| **Atlas** | 2025 | 1.2M WSIs (Mayo + Charité) | Efficient RudolfV-based model | [Paper](https://arxiv.org/abs/2501.05409) |
| **Midnight** | 2025 | 12K / 92K WSIs | Data-efficient pathology tile encoder (kaiko.ai) using a modified DINOv2 recipe that reaches state-of-the-art with orders-of-magnitude less data — Midnight-12k, trained on only 12K public TCGA WSIs, rivals models trained on 100x more; released with a larger Midnight-92k variant | [Paper](https://arxiv.org/abs/2504.05186) • [Code](https://github.com/kaiko-ai/midnight) • [Model](https://huggingface.co/kaiko-ai/midnight) |
| **OpenMidnight** | 2025 | TCGA-12K (~12K WSIs) | Fully open reproduction of Midnight (MedARC / Sophont): a ViT-G/14 tile encoder trained with DINOv2 on just ~12K TCGA WSIs for ~$1.6k, matching state-of-the-art pathology FMs with orders-of-magnitude less data | [Paper](https://sophont.med/blog/openmidnight) • [Code](https://github.com/MedARC-AI/OpenMidnight) • [Model](https://huggingface.co/SophontAI/OpenMidnight) |
| **H-optimus-0** | 2025 | 500K WSIs | Large ViT-giant (~1.1B-param) tile encoder self-supervised with DINOv2 on ~500K proprietary WSIs (Bioptimus); its companion H0-mini distills it into a far smaller model that nearly matches it at much lower inference cost, with strong robustness to staining/scanning shifts (PLISM) | [Paper](https://arxiv.org/abs/2501.16239v1) • [Code](https://github.com/bioptimus/releases/tree/main/models/h-optimus/v0) • [Model](https://huggingface.co/bioptimus/H-optimus-0) • [H-optimus-1](https://huggingface.co/bioptimus/H-optimus-1) |
| **PathOrchestra** | 2025 | 287K WSIs, 112 tasks | General-purpose pathology tile foundation model (DINOv2 SSL) trained on ~287K H&E WSIs across 21 tissues/organs; validated end-to-end across 112 clinical tasks — from slide preprocessing to pan-cancer classification, subtyping, biomarker evaluation and gene-expression prediction | [Paper](https://arxiv.org/abs/2503.24345) • [Model](https://huggingface.co/AI4Pathology/PathOrchestra) |
| **GenBio-PathFM** | 2026 | 177K WSIs | 1.1B-param open-weights histopathology FM that reaches SOTA on public benchmarks with a fraction of the training data, via (1) an automated data-curation pipeline prioritizing morphological diversity and (2) a dual-stage JEDI (DINO then JEPA) objective; the strongest open-weight model trained exclusively on public data | [Paper](https://www.biorxiv.org/content/10.64898/2026.03.17.712534v1) • [Code](https://github.com/genbio-ai/genbio-pathfm) • [Model](https://huggingface.co/genbio-ai/genbio-pathfm) |
| **RudolfV2** | 2026 | 300K+ WSIs | Family of three open-weights pathology vision foundation models narrowing the gap to closed models: a ViT-g/8 flagship (1.1B params) plus distilled ViT-B (86M) and ViT-S (22M) students; built on an extended RudolfV + Atlas recipe (DINOv2 with selective DINOv3 improvements) and a novel dedicated posttraining stage — the best-performing and most robust openly-available model, approaching the closed Atlas 2 | [Paper](https://cdn.prod.website-files.com/67adb01f31489469b513304a/6a74e2f9f1d4aef06489a3f1_RudolfV_2_260806.pdf) • [Model](https://huggingface.co/collections/Aignostics/rudolfv-2) • RudolfV 2-B • RudolfV 2-S |
...
| **Feather** | 2025 | 3,944 WSIs (BWH) | Transferable Pretrained MIL Models | [Paper](https://arxiv.org/abs/2506.09022) • [Code](https://github.com/mahmoodlab/MIL-Lab) • [Model](https://huggingface.co/collections/MahmoodLab/feather) • FEATHER-24K |
...
| Model | Year | Pretraining Data | Key Idea | Resources |
|---|---|---|---|---|
| **PLIP** | 2023 | 208K image-text pairs | CLIP-style image-text contrastive model fine-tuned on ~208K pathology image-caption pairs mined from medical Twitter (OpenPath); enables zero-shot patch classification and image/text retrieval | [Paper](https://doi.org/10.1038/s41591-023-02504-3) • [Code](https://github.com/pathologyfoundation/plip) • [Model](https://huggingface.co/vinid/plip) |
...
| **MUSK** | 2025 | 50M images + 1B text tokens | Vision-language foundation model with unified masked modeling — first pretrains on large-scale unpaired pathology images and text, then contrastively aligns image-text pairs; built for precision-oncology tasks (retrieval, VQA, biomarker and outcome prediction) | [Paper](https://www.nature.com/articles/s41586-024-08378-w) • [Code](https://github.com/lilab-stanford/MUSK) • [Model](https://huggingface.co/xiangjx/musk) |
| **TITAN** | 2025 | 335K WSIs + reports | Slide-level vision-language foundation model that aligns whole-slide embeddings with pathology reports and generated region captions (CONCH/PathChat), giving general slide representations for zero-shot and linear-probe tasks without fine-tuning | [Paper](https://www.nature.com/articles/s41591-025-03982-3) • [Code](https://github.com/mahmoodlab/TITAN) • [Model](https://huggingface.co/MahmoodLab/TITAN) |
| **CPath-Omni** | 2025 | 700k image-caption + WSI report | Unified Patch–Slide Multimodal Model | [Paper](https://arxiv.org/pdf/2412.12077) • [Code](https://github.com/PathFoundation/CPath-Omni) • [Model](https://huggingface.co/jamessyx/CPath-CLIP) |
...
| Model | Year | Pretraining Data | Key Idea | Resources |
|---|---|---|---|---|
| **STORM** | 2026 | 1.2M spatial-transcriptomics spots + matched H&E (18 organs) | Multimodal foundation model bridging spatial transcriptomics and histology: a hierarchical architecture integrates morphology, gene expression and spatial context to predict spatial gene expression from H&E, discover tissue domains, and improve immunotherapy-response prediction and prognosis; platform-agnostic across Visium, Xenium, Visium HD and CosMx | [Paper](https://arxiv.org/abs/2604.03630) • [Website](https://storm-web-demo.vercel.app/) |
| **PAST** | 2025 | 20M paired histology images + single-cell transcriptomes (pan-cancer) | Pan-cancer single-cell foundation model that jointly encodes cellular morphology and gene expression from 20M paired histopathology–single-cell-transcriptome examples, learning unified cross-modal representations for single-cell gene-expression prediction, virtual molecular staining and multimodal survival analysis directly from routine slides | [Paper](https://arxiv.org/abs/2507.06418) • [Code](https://github.com/Changchun-Yang/past_sc) |
| **HEX** | 2026 | 819K H&E tiles with matched protein expression (382 tumors) | Virtual spatial proteomics from routine H&E: built on a pathology foundation model (e.g. MUSK) to predict spatially-resolved expression of 40 immune/structural/functional protein biomarkers, enabling interpretable biomarker discovery without physical multiplex staining | [Paper](https://doi.org/10.1038/s41591-025-04060-4) • [Code](https://github.com/lilab-stanford/HEX) |
| **Tangle** | 2024 | 8.6K paired (slide + RNA-seq) | Slide-level self-supervision that contrastively aligns a WSI's multiple-instance embedding with its paired bulk transcriptome (RNA-seq), learning molecular-aware slide representations without labels | [Paper](https://arxiv.org/abs/2405.11618) • [Code](https://github.com/mahmoodlab/Tangle) |
| **Madeleine** | 2025 | 4,211 WSIs (Paired Multi-Stain) | Multi-stain slide pretraining: aligns multiple stains (H&E + IHC) of the same tissue via multimodal contrastive learning to learn stain-invariant, morphology-aware slide representations | [Paper](https://arxiv.org/pdf/2408.02859) • [Code](https://github.com/mahmoodlab/MADELEINE) • [Model](https://huggingface.co/MahmoodLab/madeleine) |
| **Threads** | 2025 | 47K paired (H&E + multi-omics) | Histomolecular slide foundation model that contrastively aligns whole-slide embeddings with paired molecular profiles (transcriptomics + genomics) to produce general slide representations | [Paper](https://arxiv.org/html/2501.16652v1) |
...
| **H2O** | 2026 | 1.3M paired H&E–spatial-omics patches | Bridges the modality gap between histology and spatial omics by CLIP-style contrastive alignment of a DINOv2 vision encoder with a fine-tuned scGPT omics encoder, letting routine H&E be read directly as a spatial transcriptomic and proteomic landscape | [Paper](https://www.biorxiv.org/content/10.64898/2026.04.21.717342v1) • [Code](https://github.com/TencentAILabHealthcare/H2O) |
| **spEMO** | 2026 | Spatial multi-omic cohorts | Rather than pretraining a new encoder, spEMO unifies frozen embeddings from pathology foundation models and LLMs into a training-flexible framework for spatial multi-omic data, and adds a multi-modal alignment task that measures how much molecular information a PFM's features actually retain | [Paper](https://www.nature.com/articles/s41551-025-01602-6) • [Code](https://github.com/HelloWorldLTY/spEMO) |
| **oFM** | 2026 | 1.67M cancer patients (clinical + DNA + RNA + H&E) | Longitudinal patient-state foundation model for precision oncology: encodes daily clinical and molecular episodes and integrates them over time together with H&E pathology into a single patient embedding, so cancer evolution and treatment history are modelled rather than a single timepoint; frozen embeddings beat expert-curated clinical/molecular features on prognosis and treatment-benefit ranking, and a mechanism-discovery framework links predicted outcomes to grounded biology via an evidence-based temporal graph | [Paper](https://arxiv.org/abs/2608.24688) |
| **DeepSpot-M** | 2026 | 730K paired histology–transcriptome spot profiles (~500 samples, 14 cancers) | Reformulates spatial-expression prediction as a query over genes: genes are represented by embeddings drawn from foundation models spanning DNA, RNA, protein, single-cell and biomedical text, so the model covers the whole protein-coding transcriptome and predicts genes never seen in training instead of a fixed panel; transfers to held-out cancers, adapts to a new cohort or single-cell assay from one slide via test-time adaptation, and the same query interface supports transcriptome restoration, cross-species non-coding RNA inference, in silico variant-effect mapping and natural-language querying | [Paper](https://www.medrxiv.org/content/10.64898/2026.06.19.26356060v1) • [Code](https://github.com/ratschlab/DeepSpotM) • [Dataset](https://huggingface.co/datasets/ratschlab/TCGA_virtual_spatial_transcriptomics_atlas) |
...
| **GPFM** | 2025 | 190M tiles, 96K WSIs | Multi-teacher distillation | [Paper](https://arxiv.org/abs/2407.18449) • [Code](https://github.com/birkhoffkiki/GPFM/) • [Model](https://huggingface.co/majiabo/GPFM) |
| **MuCoDi** | 2026 | 14.3M TCGA tiles (11.8K WSIs); distilled from 3 PFMs | Multi-teacher contrastive distillation (MoCo v3-style) that distills cached tile embeddings from Virchow2, UNI2 and H-optimus-1 into compact edge encoders (MobileOne/RepViT); students retain near-teacher accuracy at orders-of-magnitude smaller size, reaching up to 605x single-tile speedup on a Raspberry Pi 5 | [Paper](https://arxiv.org/abs/2607.05533) |
...
| **LiteFM** | 2026 | ~190M patches (72,280 WSIs); distilled from 3 PFMs | Multi-teacher distillation of Virchow2, H-Optimus-1 and UNI2 into a 22.5M-parameter ViT-S, paired with an Adaptive Patch Selector that drops task-irrelevant tiles — the two together (LitePath) make whole-slide inference cheap enough for edge hardware without giving up accuracy | [Paper](https://arxiv.org/abs/2602.14010) |

---

## 🔗 Multi-FM Fusion & Ensembling
...
| **Meta-Encoder** | 2026 | Integrates multiple pretrained PFMs | Unified integration framework that fuses features from multiple pathological foundation models into a single comprehensive representation, achieving superior downstream cancer-detection performance over any individual model | [Paper](https://doi.org/10.1038/s41467-026-71558-x) • [Code](https://github.com/ruitian-olivia/Meta-Encoder) |
| **AdaFusion** | 2025 | Adaptive inference-time fusion of multiple PFMs | Prompt-guided inference framework that dynamically integrates complementary knowledge from multiple PFMs: it compresses and aligns tile-level features and uses a lightweight attention mechanism to adaptively fuse them by tissue-phenotype context, while exposing each model's biosemantic specialisation | [Paper](https://arxiv.org/abs/2508.05084) |
| **InfoFusion** | 2025 | Fuses 8 PFMs (5 tile + 3 slide) | Information-driven fusion via correlation-guided pruning of redundant features; integrates 5 tile-level (CONCH v1.5, MUSK, Virchow2, H-optimus-1, Prov-GigaPath) + 3 slide-level (TITAN, CHIEF, MADELEINE) models for cancer grading/staging | [Paper](https://arxiv.org/abs/2512.11104) |
...
| **FuseCPath** | 2025 | Fuses multi-scale heterogeneous PFMs | Framework for fusing multi-scale, heterogeneous pathology FMs: multi-view clustering filters discriminative patches, cluster-level re-embedding fuses patch-level features, and collaborative distillation fuses slide-level FMs, yielding superior ensemble performance on WSI analysis | [Paper](https://arxiv.org/abs/2510.27237) • [Code](https://github.com/ZhidongYang/FuseCPath) |
| **Shazam** | 2026 | Fuses 5 models (UNI 2, Virchow2, H-optimus-1, Prov-Gigapath, Phikon-v2) | Adaptive integration/distillation | [Paper](https://arxiv.org/abs/2503.00736) • [Code](https://github.com/Tuner12/Shazam) |
...
| Model | Year | Pretraining Data | Key Idea | Resources |
|---|---|---|---|---|
| **CytoSyn** | 2026 | >10K TCGA WSIs (32 cancer types) | Foundation latent-diffusion model for histopathology that guides generation of highly realistic, diverse H&E images; the improved CytoSyn-v2 is benchmarked in depth against PixCell and generalizes to inflammatory-bowel-disease images despite oncology-only training, targeting tasks beyond feature extractors such as virtual staining | [Paper](https://arxiv.org/abs/2603.18089) • [Model](https://huggingface.co/Owkin-Bioptimus/CytoSyn) |
...
## 📊 General Benchmarking & Leaderboards

| Name | Year | Scope | Key Contribution | Resources |
|---|---|---|---|---|
| **EVA** | 2024 | Slide & Patch | Open, reproducible evaluation framework for pathology FMs — runs a fixed suite of patch- and slide-level tasks under standardized protocols so encoders can be compared consistently | [Paper](https://openreview.net/forum?id=FNBQOPj18N) • [Code](https://github.com/kaiko-ai/eva) |
| **Patho-Bench** | 2025 | Slide | Open-source evaluation toolkit with standardized datasets and reproducible pipelines for comparing pathology FMs | [Paper](https://arxiv.org/pdf/2502.06750) • [Code](https://github.com/mahmoodlab/Patho-Bench) |
| **PathBench** | 2025 | Slide & Patch | Leakage-free multi-center clinical evaluation spanning multiple tasks, with a live public leaderboard | [Paper](https://arxiv.org/abs/2505.20202) • [Code](https://github.com/birkhoffkiki/PathBench) • [Website](https://birkhoffkiki.github.io/PathBench/) |
| **Stanford PathBench** | 2026 | Slide & Patch | Evaluates 32 PFMs across 41 tasks with a public leaderboard | [Paper](https://www.nature.com/articles/s41467-026-76004-6) |
| **QuIIL-PathBench** | 2025 | Patch | Evaluates pathology FMs at patch level across adaptation strategies (e.g., linear probing vs fine-tuning) and deployment scenarios, quantifying how those choices affect downstream performance | [Paper](https://arxiv.org/abs/2410.16038) • [Code](https://github.com/QuIIL/BenchmarkingPathologyFoundationModels) |
| **Clinical Benchmark** | 2025 | Slide | Clinical evaluation on MSHS and MSKCC datasets covering disease detection and biomarker prediction | [Paper](https://www.nature.com/articles/s41467-025-58796-1.pdf) • [Code](https://github.com/sinai-computational-pathology/SSL_tile_benchmarks#automated-external-benchmarking) |
| **Feature-Extractor Benchmark** | 2025 | Slide | Evaluates 19 FMs as feature extractors for weakly-supervised slide tasks; CONCH best, CONCH+Virchow2 ensemble tops 55% of tasks | [Paper](https://www.nature.com/articles/s41551-025-01516-3) • [Code](https://github.com/KatherLab/STAMP) • [Website](https://zenodo.org/records/15749283) |
| **HistoVL** | 2025 | Patch (VLM) | Holistic evaluation of histopathology VLMs across 26 organs, 31 cancer types, >5M patches from 41K+ WSIs (11 scanners); tests text-change sensitivity, adversarial robustness and calibration | [Paper](https://arxiv.org/abs/2503.12990) |

---

## 📈 Organ- & Task-Specific Benchmarking
...
| **Kidney FM Benchmark** | 2026 | Slide & Patch (kidney) | Evaluates 11 PFMs on 11 kidney histopathology tasks; PFMs encode coarse/meso-scale morphology well but struggle with subtle microstructure, complex phenotypes and prognosis | [Paper](https://arxiv.org/abs/2603.15967) |
| **Breast Survival Benchmark** | 2026 | Slide (breast) | Evaluates PFMs for breast-cancer survival across 3 cohorts (>5,400 patients); second-gen models improve but recent models differ modestly, and H0-mini can slightly beat its larger teacher (diminishing scaling returns) | [Paper](https://arxiv.org/abs/2604.24679) |
| **MAKO** | 2026 | Slide (breast) | Evaluates 12 PFMs (+2 non-pathology baselines) for interpretable breast-cancer recurrence-risk (ROR-P) prediction from H&E WSIs via attention-based MIL | [Paper](https://www.nature.com/articles/s41746-025-02334-2) • [Code](https://github.com/kaczmarj/mako) |
| **PANDA-PLUS-Bench** | 2025 | 9 expert-annotated prostate WSIs; 7 FMs | Clinical benchmark that quantifies whether prostate-cancer FMs learn generalizable biology vs specimen-specific artifacts in Gleason (GP3/GP4) grading: nine expert-annotated biopsies, non-overlapping patches at 512 and 224 px across eight augmentations, evaluating cross-slide accuracy vs slide-level encoding for seven foundation models | [Paper](https://arxiv.org/abs/2512.14922) |
| **Segmentation FM Benchmark** | 2026 | Patch (segmentation) | Evaluates 10 FMs on 4 semantic-segmentation datasets using attention heads as pixel representations; differently-trained PFMs are complementary — fusing CONCH, PathDino and CellViT improves performance | [Paper](https://arxiv.org/abs/2602.18747) |
...
|---|---|---|---|
| **ConceptM3oE** — *ConceptM3oE: Concept-Guided Multimodal Mixture of Experts for Interpretable Computational Pathology* | 2026 | Concept-guided multimodal mixture-of-experts for interpretable pathology: decomposes evidence (WSI + report + molecular) into modality-specific, redundant and synergistic experts projected into morphology/biomarker concept bottlenecks, with residual pathways preserving accuracy; yields neuropathologist-validated reasoning traces and improves low-data macro-F1 (56.4%->66.7%) | [Paper](https://arxiv.org/abs/2605.24399) |
| **DICE** — *Uncertainty Estimation in Pathology Foundation Models via Deep Mutual Learning* | 2026 | Uncertainty estimation for pathology FMs via deep mutual learning: DICE ensembles K frozen PFMs and aligns them so their disagreement upper-bounds model uncertainty, giving reliable confidence for WSI classification, calibration and patch-level abnormality localization under in- and out-of-distribution shift without localization supervision | [Paper](https://arxiv.org/abs/2606.30020) |
...
| **Vig et al.** — *Do Pathology Foundation Models Encode Disease Progression? A Pseudotime Analysis of Visual Representations* | 2026 | Applies diffusion pseudotime to PFM embeddings; 5 models recover continuous cancer-progression trajectories, and trajectory fidelity predicts few-shot generalization | [Paper](https://arxiv.org/abs/2601.21334) • [Code](https://github.com/pritika-vig/embeddings_disease_progression_analysis) |
| **PICASSO** — *Dissecting and Directing Pathology Foundation Models* | 2026 | A sparse autoencoder decomposes PFM embeddings into human-interpretable histomorphological concepts, yielding a pan-cancer concept atlas that supports not just auditing but concept-level intervention — suppressing artifact concepts or generating counterfactual embeddings | [Paper](https://www.biorxiv.org/content/10.64898/2026.06.12.731496v1) |
...
| **Ndubuisi et al.** — *What Carries the Signal in Pathology Foundation-Model Atlases? A Patient-Level Controlled Benchmark in Breast Cancer* | 2026 | Patient-controlled TCGA-BRCA analysis across 11 frozen backbones; some molecular signal is genuine, but much is explained by interpretable morphology | [Paper](https://arxiv.org/abs/2608.00105) |
| **STpath** — *Translating Histopathology Foundation Model Embeddings into Cellular and Molecular Features for Clinical Studies* | 2026 | Translates opaque PFM embeddings into biology clinicians can act on: cancer-specific XGBoost heads map image-tile embeddings to cell-type composition and gene expression using matched spatial transcriptomics, so latent features become quantities that can be tested against clinical outcomes | [Paper](https://www.biorxiv.org/content/10.64898/2026.03.17.711896v1) • [Code](https://github.com/Sun-lab/STpath-CRC) |
...
| Paper | Year | Focus | Resources |
|---|---|---|---|
| **PathoROB** — *Towards robust foundation models for digital pathology* | 2026 | Robustness benchmark: 20 PFMs, 99,392 patches, 28 biological classes, 34 centers; compares biological information vs technical confounding and finds substantial robustness deficits | [Paper](https://www.nature.com/articles/s41467-026-73923-2) • [Dataset](https://huggingface.co/collections/bifold-pathomics/pathorob) |
...
| **PLISM** — *Registered multi-device/staining histology image dataset for domain-agnostic machine learning models* | 2025 | Turns the PLISM registered multi-scanner/multi-stain dataset into a robustness benchmark: because the same tissue is imaged under every stain-device combination and the tiles are pixel-aligned, feature drift under domain shift can be measured directly rather than inferred from downstream accuracy | [Paper](https://www.nature.com/articles/s41597-024-03122-5) • [Code](https://github.com/owkin/plism-benchmark) • [Dataset](https://huggingface.co/datasets/owkin/plism-dataset) • [Website](https://owkin.github.io/plism-benchmark/) |
